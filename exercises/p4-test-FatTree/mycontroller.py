...
'''def printRegister(p4info_helper, sw, register_name, index):
    for response in sw.ReadRegister(p4info_helper.get_register_id(register_name), index):
        for entity in response.entities:
...
def main(p4info_file_path, bmv2_file_path):
    ...
    try:
        ...
        # Print the tunnel counters every 1 seconds
        while True:
            sleep(1)
            print('\n----- Reading tunnel counters -----')
            printCounter(p4info_helper, s1, "Counter_pb", 4)
            printCounter(p4info_helper, s2, "Counter_pb", 4)
            printCounter(p4info_helper, s3, "Counter_pb", 4)
            printCounter(p4info_helper, s4, "Counter_pb", 4)
        #    printRegister(p4info_helper, s4, "pkt_num_reg", 0)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--p4info', help='p4info proto in text format from p4c',
                        ...
                        default='./build/test-1.p4.p4info.txt')
    parser.add_argument('--bmv2-json', help='BMv2 JSON file from p4c',
                        type=str, action="store", required=False,
                        default='./build/test-1.json')
